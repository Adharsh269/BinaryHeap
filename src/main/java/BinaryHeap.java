public class BinaryHeap {
  int[] arr;
  int sizeOfTree;
  public BinaryHeap(int capacity) {
    this.arr = new int[capacity+1];
    this.sizeOfTree = 0;
  }

  //isEmpty method
  public boolean isEmpty(){
    return sizeOfTree==0;
  }
  public boolean isFull(){
    return arr.length-1==sizeOfTree;
  }
  //peek method
  public Integer peek(){
    if(isEmpty()){
      System.out.println("Binary Heap is Empty");
      return null;
    }
    return arr[1];
  }

  //size 
  public int size(){
    return sizeOfTree;
  }
  //level order
  public void levelOrder(){
    for(int i=1;i<=sizeOfTree;i++){
      System.out.print(arr[i]+" ");
    }
    System.out.println();
  }

  //helper function for inserting an element to the binary heap
  public void heapifyBottomToTop(int index,String heapType){
    int parent=index/2;
    if(index<=1) return;

    if(heapType=="Min"){
      if(arr[index]<arr[parent]){
        int temp=arr[index];
        arr[index]=arr[parent];
        arr[parent]=temp;
      }
    }else if(heapType=="Max"){
      if(arr[index]>arr[parent]){
        int temp=arr[index];
        arr[index]=arr[parent];
        arr[parent]=temp;
      }
      heapifyBottomToTop(parent, heapType);
    }
  }
  //insert method
  public void insert(int value,String heapType){
    if(isFull()){
      System.out.println("Tree is full");
      return;
    }
    arr[sizeOfTree+1]=value;
    sizeOfTree++;
    heapifyBottomToTop(sizeOfTree, heapType);

    System.out.println("Value :"+value+" is successfully inserted.");
  }

  public void heapifyTopToBottom(int index,String heapType){
    int left=index*2;
    int right=index*2+1;
    int swapChild=0;

    if(sizeOfTree<left) return;

    if(heapType=="Max"){
      if(sizeOfTree==left){
        if(arr[index]<arr[left]){
          int temp=arr[index];
          arr[index]=arr[left];
          arr[left]=temp;
       }
        return;
      }else {
        if(arr[left] > arr[right]){
          swapChild=left;
        }else{
          swapChild=right;
        }
        if(arr[index] < arr[swapChild]){
          int temp=arr[index];
          arr[index]=arr[swapChild];
          arr[swapChild]=temp;
        }
      }
    }else if(heapType=="Min"){
      if(sizeOfTree==left){
        if(arr[index]>arr[left]){
          int temp=arr[index];
          arr[index]=arr[left];
          arr[left]=temp;
       }
        return;
      }else {
        if(arr[left] < arr[right]){
          swapChild=left;
        }else{
          swapChild=right;
        }
        if(arr[index] > arr[swapChild]){
          int temp=arr[index];
          arr[index]=arr[swapChild];
          arr[swapChild]=temp;
        }
      }
    }
    heapifyTopToBottom(swapChild, heapType);
  }

  public int extractHeadNode(String heapType){
    if(isEmpty()) return -1;
    int extractValue=arr[1];
    arr[1]=arr[sizeOfTree];
    sizeOfTree--;
    heapifyTopToBottom(1, heapType);
    System.out.println("Node has been successfully deleted.");
    return extractValue;
  }

  public void deleteBH(){
    arr=null;
    sizeOfTree=0;
    System.out.println("BinaryTree has been successfully deleted.");
  }
}